...
def plot_augm_masks(batch):
    ...
    titles = 5 * ['image'] + 5 * ['mask']
    my_iter = zip(axis.flatten(), chain(batch.images[:5], batch.masks[:5]), titles)
    for ax, image, title in my_iter:
...
def plot_pred_mask(test_batch, pred):
    ...
    titles = 3 * ['image'] + 3 * ['mask'] + 3 * ['predicted']
    iterator = chain(test_batch.images[:3], test_batch.masks[:3], pred[:3])
    for ax, image, title in zip(axis.flatten(), iterator, titles):
        if image.shape[0] == 3:
