...
class MnistBatch(ImagesBatch):
    ...
    def convy():
        ...
        return [[x, y_, loss, train_step, training, keep_prob], [labels, labels_hat, accuracy], [probs],
                [learning_rate, global_step, print_loss], [softmax, predicts, test_acc]]
...
#             saver.save(sess, 'trained/model', global_step=global_step)
    ...
    def update_stats_ensemble(self, model, accs, loss_history):
        ensemble_data = model
        results = []
    ...
    #             res = new_sess.run(softmax_output, feed_dict={x:self.images, training:False})
    #             results.append(res)
