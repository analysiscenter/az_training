...
class NoisedBatch(ImagesBatch):
    ...
    def random_location(self, image, *args):
        ...
        image_size = args[0]
        imsize = self.images.shape[1]
        filt = self.images.shape[-1]
        pure_mnist = np.squeeze(image)
        new_x, new_y = np.random.randint(0, image_size-imsize, 2)
        large_mnist = np.zeros((image_size, image_size, filt))
        large_mnist[new_x:new_x+imsize, new_y:new_y+imsize] = pure_mnist
        return large_mnist.reshape(image_size, -1, filt), [new_x, new_y]
    ...
    def create_noise(self, image, *args):
        ...
        if args[0] == 'random_noise':
            noise = (args[1] * np.random.random((image_size, image_size, filt)) * image.max()).reshape(-1, image_size, filt)
            ...
            fragments = self.create_fragments(images, size)
            noise = self.arrange_fragments(image_size, fragments, distr, level, filt)
        else:
    ...
    def arrange_fragments(self, image_size, fragments, distr, level, filt):
        """Put fragments on image."""
        image = np.zeros((image_size, image_size, filt))
        for fragment in fragments:
            ...
            height_to_change, width_to_change, _ = image_to_change.shape
