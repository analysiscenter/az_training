...
class FocalBatch(ImagesBatch):
    ...
    def resnet(self, config):
        ...
        with tf.Graph().as_default():
            gamma = tf.constant(self.pipeline.config['resnet']['gamma'])
            ...
            # loss = tf.reduce_mean(-tf.reduce_sum(y_ * tf.pow((1 - prob1), gamma) *tf.log(prob1), reduction_indices=[1]))

            focal_factor = tf.pow((1 - prob1), gamma)
            loss = tf.reduce_mean(-tf.reduce_sum(y_ * focal_factor * tf.log(prob1 + 1e-7), reduction_indices=[1]))
            ...
            labels_hat = tf.cast(tf.argmax(prob1, axis=1), tf.float32, name='labels_hat')
        ...
        return [[indices, all_data, all_lables, loss, train1, prob1], [accuracy1, session], [training_mode, drop_rate], [focal_factor]]
    ...
    def train_res(self, models, train_loss, accs, data, lables, time_list, rate):
        ...
        # f_c = sess.run(focal_factor, feed_dict={indices:self.indices.reshape(-1, 1), \
        #     all_lables:lables, all_data:data, training:True, drop_rate:rate})
