...
class MnistBatch(ImagesBatch):
    ...
    def resnet(self, config):
        ...
        with tf.Graph().as_default():
            ...
            # net1 = conv_block(net1, 3, [32, 32, 128], name='in_34', strides=(1, 1))
            ...
            # train1 = tf.train.MomentumOptimizer(learning_rate, 0.7, use_nesterov=True).minimize(loss, global_step=global_step)

            train1 = (tf.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step))
            # train1 = (tf.train.AdamOptimizer().minimize(loss, global_step=global_step))
        ...
        return [[indices, all_data, all_lables, loss, train1, prob1], [accuracy1, session], [learning_rate, global_step]]
    ...
    def train_res(self, models, train_loss, accs, data, lables):
        ...
        with grph.as_default():
            ...
            sess.run(train, feed_dict={indices:self.indices.reshape(-1, 1), all_lables:lables, all_data:data, learning_rate:cyclic_learning_rate})   

            period = sess.run(period)
            global_step = sess.run(global_step)

            # print ('WATCH OUT global_step', global_step, 'period ',  period)

            if (global_step) % period == 0:
                print('entered')
                print (global_step, period)
                if global_step == 0:
                    print('0000')
                    pass
                else:
    ...
    def update_stats_ensemble(self, model, config, accs, loss_history, all_data, all_labels):
        ...
        for i, sess in enumerate(ensemble_data['sess']):
            ...
            results.append(sess.run(softmax_output, feed_dict={indices:self.indices.reshape(-1, 1), all_data_placehoder:all_data}))
        
        # print(len(results))
        # print('shape', results[0].shape)
        avg_logits = np.max(np.array(results), axis=0)
        # print('pr', avg_logits)
