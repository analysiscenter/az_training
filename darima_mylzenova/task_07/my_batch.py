...
class MnistBatch(ImagesBatch):
    ...
    def load(self, src, fmt='blosc'):
        ...
        if fmt == 'blosc':     
            ...
            with open(os.path.join(src, 'mnist_pics.blk'), 'rb') as file:
                ...
                self.images = np.reshape(self.images, (-1, 28, 28))
        ...
        if fmt == 'int8':
            ...
            # self.images = self.images*256 -128
    ...
    def convy_float(self):
        ...
        with graph.as_default():
            ...
            # x = tf.placeholder(tf.int8, [None, 28, 28])
            # x_as_pics = tf.cast(x, tf.float32)
            # x_as_pics = tf.reshape(x_as_pics, shape=[-1, 28, 28, 1])

            dim_size = 400  
            x = tf.placeholder(tf.float32, [None, None, None])
            # x = tf.placeholder(tf.float32, [None, dim_size, dim_size])
            x_as_pics = tf.reshape(x, shape=[-1, dim_size, dim_size, 1])
        ...
        return [[x, y_, loss, train_step, training, keep_prob], [labels, labels_hat, accuracy], [probs], [learning_rate], [sess]]
    ...
    def convy_int(self):
        ...
        with graph.as_default():
            ...
            dim_size = 400
            x = tf.placeholder(tf.int8, [None, None, None])
            x_as_pics = tf.cast(x, tf.float32)
            x_as_pics = tf.reshape(x_as_pics, shape=[-1, dim_size, dim_size, 1])
    ...
    @action(model='convy_float')
    def train_convy_float(self, model, time_list):
    ...
    @action(model='convy_int')
    def train_convy_int(self, model, time_list):
        """ Train-action for convy-model
...
#     @action(model='convy')
#     def update_stats(self, model, accs, lr_history):
