...
class MnistBatchBottleneck(ImagesBatch):
    ...
    def resnet(self, config):
        ...
        with tf.Graph().as_default():
            ...
            net1 = conv_block(net1, 3, [8, 8, 16], name='22', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
            
            # for block_length in range(length_factor):
            #     net1  = identity_block(net1, 3, [16, 16], name=str(block_length) +'n1', initializer=conv_initializer, w_factor=widening_factor)

            net1 = conv_block(net1, 3, [16, 16, 32], name='33', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
            
            # for block_length in range(length_factor):
            #     net1  = identity_block(net1, 3, [32, 32], name=str(block_length) +'n2', initializer=conv_initializer, w_factor=widening_factor)

            # net1 = conv_block(net1, 3, [64, 64], name='53', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
            net1 = conv_block(net1, 3, [32, 32, 64], name='63', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
        
            # for block_length in range(length_factor):
            #     net1  = identity_block(net1, 3, [64, 64], name=str(block_length) +'n3', initializer=conv_initializer, w_factor=widening_factor)
            ...
            for add_block in range(additional_blocks):
                ...
                net1  = identity_block(net1, 3, [32, 64, 64], name=str(add_block) +'id_n3', initializer=conv_initializer, w_factor=widening_factor)
            ...
            # net1 = conv_block(net1, 3, [32, 32, 128], name='22', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)

            # net1 = identity_block(net1, 3, [32, 32, 128], name='33', initializer=conv_initializer, w_factor=widening_factor)

            # net1 = conv_block(net1, 3, [64, 64, 256], name='53', strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
            # net1 = identity_block(net1, 3, [64, 64, 256], name='63', initializer=conv_initializer, w_factor=widening_factor)

            # for add_block in range(additional_blocks):
            #     # net1 = conv_block(net1, 3, [64, 64], name='73_'+ str(add_block), strides=(1, 1), initializer=conv_initializer, w_factor=widening_factor)
            #     net1  = identity_block(net1, 3, [64, 64], name=str(add_block) +'id_n3', initializer=conv_initializer, w_factor=widening_factor)
    ...
    @action(model='resnet')
    def update_stats(self, models, test_loss, accs, data, lables):
...
def conv_block(input_tensor, kernel, filters, name, initializer, strides=(2, 2), w_factor=1):
    ...
    print(filters1, filters2, filters3)
...
def identity_block(input_tensor, kernel, filters, name, initializer, w_factor):
